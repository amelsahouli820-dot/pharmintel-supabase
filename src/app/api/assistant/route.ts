...
export async function POST(request: NextRequest) {
  ...
  const records = await db.intelligenceRecord.findMany({
    ...
    select: { id: true, observedAt: true, wholesaler: true, laboratory: true, product: true, productRange: true, molecule: true, therapeuticClass: true, productCode: true, cip: true, price: true, priceHt: true, priceTtc: true, promotionalPrice: true, currency: true, offerType: true, discountPercent: true, ugPercent:true,freeUnits: true,offerBuyQty:true,offerFreeQty:true,offerText:true,netPrice:true,priceAfterUg:true,savings:true, quota: true, commercialConditions: true, startsAt: true, endsAt: true, wilaya: true, city: true, region: true, salesperson: true, distributionChannel: true, comments: true, document:{select:{watchType:{select:{name:true,code:true}},customWatchType:true}}, user: { select: { name: true } } }
  });
  const dataset = records.map(r => ({ ...r, price: r.price?.toString(), priceHt: r.priceHt?.toString(), priceTtc: r.priceTtc?.toString(), promotionalPrice: r.promotionalPrice?.toString(), discountPercent: r.discountPercent?.toString(),ugPercent:r.ugPercent?.toString(),netPrice:r.netPrice?.toString(),priceAfterUg:r.priceAfterUg?.toString(),savings:r.savings?.toString() }));
