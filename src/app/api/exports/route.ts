...
const columns = ["Date","Grossiste","Laboratoire","Produit","Gamme","Molécule","Classe thérapeutique","CIP","Code produit","Prix HT","Prix TTC","Prix promotionnel","Devise","Type","Type de veille","Remise","UG","Quota","Début","Fin","Conditions commerciales","Wilaya","Ville","Région","Commercial","Canal","Commentaires","Utilisateur"];
function values(r: Row) { return [r.observedAt?.toISOString().slice(0,10)||"",r.wholesaler||"",r.laboratory||"",r.product,r.productRange||"",r.molecule||"",r.therapeuticClass||"",r.cip||"",r.productCode||"",r.priceHt?.toString()||"",r.priceTtc?.toString()||"",r.promotionalPrice?.toString()||r.price?.toString()||"",r.currency,r.offerType,r.document.watchType?.name||"",r.discountPercent?`${r.discountPercent}%`:"",r.freeUnits??"",r.quota||"",r.startsAt?.toISOString().slice(0,10)||"",r.endsAt?.toISOString().slice(0,10)||"",r.commercialConditions||"",r.wilaya||"",r.city||"",r.region||"",r.salesperson||"",r.distributionChannel||"",r.comments||"",r.user.name]; }
