...
export async function POST(request:NextRequest,{params}:C){
 ...
 const watchRef=await db.referenceEntity.findFirst({where:{id:m.watchTypeId,type:"WATCH_TYPE",active:true}});if(!watchRef)return badRequest("Sélectionnez un type de veille valide.");if(watchRef.code==="OTHER"&&!m.customWatchType)return badRequest("Précisez le type de veille.");
